...
export default function SessionsPage() {
  ...
  const { data = [], isLoading } = useQuery({
    queryKey: ["sessions", page, search, option],
    queryFn: async () => {
      const res = await axios.get("/sessions/allApSessions", {
        params: {
          page,
          limit: 10,
          search,
          ...
          >
            <div className="card-body space-y-2">
              ...
              <p className="text-gray-600 text-sm">
                {session.sessionDescription?.slice(0, 80)}...
              </p>

              <div className="text-sm text-gray-700 space-y-1">
                <p className="flex items-center gap-2">
                  <User2 size={16} className="text-indigo-500" />
                  <strong>{session.tutorName}</strong> ({session.tutorEmail})
                </p>
                <p className="flex items-center gap-2">
                ...
                <p className="flex items-center gap-2">
                  ...
                  {session.isPaid ? `$${session.registrationFee}` : "Free"}
                </p>
                <p className="flex items-center gap-2">
